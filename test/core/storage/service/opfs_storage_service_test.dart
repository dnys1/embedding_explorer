...
void main() {
  ...
  group('StorageService', () {
    test('defaults to worker correctly', testOn: '!safari', () async {
      expect(await OpfsStorageService.shouldUseWorker, isFalse);
    });

    test('defaults to worker correctly', testOn: 'safari', () async {
      expect(await OpfsStorageService.shouldUseWorker, isTrue);
    });

    for (final useWorker in [true, false]) {
      group(
        'OpfsStorageService (worker=$useWorker)',
        onPlatform: !useWorker
            ? {'safari': Skip('createWritable not supported')}
            : null,
        () {
          late StorageService storageService;

          setUp(() async {
            storageService = await StorageService.opfs(useWorker: useWorker);
          });
          ...
          test('has correct name', () {
            expect(storageService.name, contains('OPFS'));
          });
          ...
          group('File deletion', () {
            test('should delete existing file', () async {
              ...
              expect(
                () => storageService.readAsBytes(testPath),
                throwsA(anything),
              );
            });

            test('should not throw when deleting non-existent file', () async {
              expect(storageService.delete('non-existent-file.txt'), completes);
            });
          });
          ...
          group('Storage clearing', () {
            test('should clear all files', () async {
              ...
              for (final path in testFiles.keys) {
                expect(
                  () => storageService.readAsString(path),
                  throwsA(anything),
                );
                expect(
                  () => storageService.readAsBytes(path),
                  throwsA(anything),
                );
              }
            });
          ...
          group('Error handling', () {
            ...
            test('should handle special characters in file paths', () async {
              ...
              final retrievedData = await storageService.readAsString(
                specialPath,
              );
            ...
            test(
              ...
              onPlatform: {'safari': Skip('Throws an OOM exception')},
              () async {
