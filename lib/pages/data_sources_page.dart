...
import '../models/data_sources/data_source_config.dart' as config;
...
class _DataSourcePageState extends State<DataSourcesPage> {
  ...
  void _saveDataSource(DataSource dataSource) {
    // Convert DataSource back to DataSourceConfig for storage
    final configDataSource = config.DataSourceConfig(
      id: _selectedDataSource?.id ?? _configManager.dataSources.generateId(),
      name: dataSource.name,
      description: '', // DataSource doesn't have description, use empty string
      type: dataSource.type == 'csv'
          ? config.DataSourceType.csv
          : config.DataSourceType.sqlite,
      settings: Map<String, dynamic>.from(dataSource.settings),
      createdAt: DateTime.now(), // TODO
      updatedAt: DateTime.now(),
    );

    _configManager.dataSources.set(configDataSource.id, configDataSource);
    _hideDialog();
  }

  void _deleteDataSource(config.DataSourceConfig dataSource) {
    _configManager.dataSources.remove(dataSource.id);
  }
