...
abstract class WorkerBeeCommon<Request extends Object, Response>
  ...
  void _handleLogRecord(LogRecord log) {
    logSink.sink.add(WorkerLogRecord.from(log, local: false));
    if (logsController.isClosed) return;
    logsController.add(WorkerLogRecord.from(log, local: !isRemoteWorker));
  }
  ...
  @protected
  final StreamSinkCompleter<LogRecord> logSink = StreamSinkCompleter();

  /// Configures logging for the worker.
  set _logsChannel(StreamChannel<LogRecord> channel) {
  ...
  @protected
  final StreamController<LogRecord> logsController = StreamController.broadcast(
    sync: true,
  );

  /// The logger to use for all messages.
  Stream<LogRecord> get logs => logsController.stream;
  ...
  @mustCallSuper
  Future<void> connect({StreamChannel<LogRecord>? logsChannel}) async {
    _isRemoteWorker = true;
    if (logsChannel != null) {
      _logsChannel = logsChannel;
    }
    logger.fine('Connected from worker');
  }
  ...
  void completeError(Object error, [StackTrace? stackTrace]) {
    logger.fine('Error in worker', error, stackTrace);
    if (!isCompleted) {
  ...
  Future<void> close({bool force = false}) => _closeMemoizer.runOnce(() async {
    logger.fine('Closing worker (force=$force)');
    await Future.wait(
