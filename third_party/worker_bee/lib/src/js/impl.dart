...
mixin WorkerBeeImpl<Request extends Object, Response>
  ...
  StreamChannel<LogRecord>? _logsChannel;
  ...
  Future<void> spawn({String? jsEntrypoint}) async {
    return runTraced(() async {
      for (final entrypoint in [
        ...
        logger.fine('Spawning worker at $entrypoint');

        // Spawn the worker using the specified script.
        try {
          _worker = Worker(entrypoint.toJS);
        } on Object {
          logger.fine('Could not launch worker at $entrypoint');
          continue;
        }
        ...
        try {
          ...
          _logsChannel = MessagePortChannel<LogRecord>(
            jsLogsChannel.port1,
          ...
          logger.fine('Error initializing worker', e, st);
          continue;
        }
      }
